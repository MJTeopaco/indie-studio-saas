"""
Validates task_node_features_v2.csv — Task Node Feature Matrix (X_task) v2.
Run: python validate_task_dataset_v2.py
"""
...
PATH = os.path.join(os.path.dirname(__file__), "task_node_features_v2.csv")
...
CLASSIFICATION_MACRO = {
    "Product Requirements & Analysis": "Product Strategy & Management",
    "Sprint & Roadmap Planning": "Product Strategy & Management",
    "Market Viability Research": "Product Strategy & Management",
    ...
    "Third-Party API Setup": "Web & SaaS Platforms",
    ...
    "DevSecOps & Security Auditing": "DevOps & IT Infrastructure",
    "Hardware-in-the-Loop Testing": "Hardware Prototyping & Embedded Systems",
    "Hardware Sensor Integration": "Hardware Prototyping & Embedded Systems",
    ...
    "System Mechanics Planning": "UI/UX & Digital Asset Design",
...
try:
    ...
    print("ERROR: File not found. Run generate_task_dataset_v2.py first.")
    raise SystemExit(1)
...
# ── 3. Sprint ID correctness ──────────────────────────────────────────────────
sprint_errors = []
for r in rows:
    tid = int(r["task_id"])
    sid = int(r["sprint_id"])
    expected_sid = math.ceil(tid / 200)
    if sid != expected_sid:
        sprint_errors.append(f"  task {tid}: sprint_id={sid} expected={expected_sid}")
print(f"{'PASS' if not sprint_errors else 'FAIL'} sprint_id formula ceil(task_id / 200): "
      f"{len(sprint_errors)} violation(s)")
errors += len(sprint_errors)

# ── 4. Skill sparsity: exactly 1–4 non-zero req_skill_* per row ───────────────
sparse_errors = []
for r in rows:
    tid = r["task_id"]
...
# ── 7. days_until_deadline range 1–60 ────────────────────────────────────────
deadline_errors = []
for r in rows:
...
# ── 8. Predecessor IDs strictly < task_id ────────────────────────────────────
pred_errors = []
for r in rows:
    tid = int(r["task_id"])
...
# ── 10. Classification → macro_domain consistency (all rows) ─────────────────
macro_errors = []
for r in rows:
...
# ── 12. No dead skill columns (every skill is active in at least one row) ────
dead_skills = [c for c in skill_cols if all(int(r[c]) == 0 for r in rows)]
print(f"{'PASS' if not dead_skills else 'FAIL'} no dead skill columns: "
      f"{len(dead_skills)} dead column(s){' -> ' + str(dead_skills) if dead_skills else ''}")
errors += len(dead_skills)
